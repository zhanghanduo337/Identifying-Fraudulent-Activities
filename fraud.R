...
df_ip = read.csv('/Users/zhanghanduo/Desktop/data/Fraud/IpAddress_to_Country.csv')
df_fraud = read.csv('/Users/zhanghanduo/Desktop/data/Fraud/Fraud_Data.csv')
attach(df_ip)
attach(df_fraud)
...
p <- ggplot(df, aes(x=class, y=time_interval,group = class)) + 
...
roc = roc.curve(scores.class0=rf_scores[scores$class=="1",]$score,
         scores.class1=rf_scores[scores$class=="0",]$score,
         curve=T)
plot(roc)
...
pr = pr.curve(scores.class0=rf_scores[scores$class=="1",]$score,
               scores.class1=rf_scores[scores$class=="0",]$score,
               curve=T)
plot(pr)
